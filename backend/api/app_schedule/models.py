...
class Schedule(models.Model):
    ...
    schedule_id = models.CharField(
        max_length=20, primary_key=True, unique=True, verbose_name=_("Mã lịch học"), default="LH001"
    )
    class_assigned = models.ForeignKey(
        Class, on_delete=models.CASCADE, related_name="schedules", verbose_name=_("Lớp học")
    )
    ...
    semester = models.ForeignKey(
        Semester, on_delete=models.CASCADE, related_name="schedules", verbose_name=_("Học kỳ")
    )
    ...
    day_of_week = models.CharField(
        max_length=3, choices=DAY_CHOICES, verbose_name=_("Ngày trong tuần")
    )
    ...
    created_at = models.DateTimeField(
        auto_now_add=True, verbose_name=_("Ngày tạo"), null=False, blank=False
    )
    updated_at = models.DateTimeField(auto_now=True, verbose_name=_("Ngày cập nhật"))

    def __str__(self):
        return f"{self.class_assigned} - {self.day_of_week} {self.start_time}-{self.end_time}"

    class Meta:
        app_label = "app_schedule"
