...
class EnrollmentForm(forms.ModelForm):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.fields["class_obj"].queryset = Class.objects.filter(is_active=True).order_by(
            "class_id"
        )
    ...
    def clean_enrollment_date(self):
        ...
        if enrollment_date:
            ...
            if self.cleaned_data.get("semester"):
                ...
                if (
                    enrollment_date < semester.registration_start
                    or enrollment_date > semester.registration_end
                ):
                    raise ValidationError(
                        _("Ngày đăng ký phải nằm trong khoảng thời gian đăng ký của học kỳ.")
                    )
    ...
    def clean(self):
        ...
        if class_obj and subject and semester:
            if not Class.objects.filter(
                subject=subject, semester=semester, class_id=class_obj.class_id
            ).exists():
