...
class IsAdminOrTeacher(BasePermission):
    ...
    def has_permission(self, request, view):
        return request.user and (
            getattr(request.user, "is_admin", False) or getattr(request.user, "is_teacher", False)
        )
...
class CanManageScores(BasePermission):
    ...
    def has_permission(self, request, view):
        ...
        permission = Permission.objects.filter(
            codename="can_manage_scores", content_type=content_type
        ).first()
...
class CanManageSubject(BasePermission):
    ...
    def has_permission(self, request, view):
        ...
        content_type = ContentType.objects.get_for_model(view.model)
        permission = Permission.objects.filter(
            codename="can_manage_subject", content_type=content_type
        ).first()
...
class CanViewSubjectScores(BasePermission):
    ...
    def has_permission(self, request, view):
        ...
        content_type = ContentType.objects.get_for_model(view.model)
        permission = Permission.objects.filter(
            codename="can_view_subject_scores", content_type=content_type
        ).first()
        return permission and request.user.has_perm(
            f"{content_type.app_label}.can_view_subject_scores"
        )
