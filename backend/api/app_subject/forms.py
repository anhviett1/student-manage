...
class SubjectForm(forms.ModelForm):
    class Meta:
        model = Subject
        fields = [
            'subject_id', 'name', 'description', 'credits',
            'semester', 'department', 'status', 'is_active',
        ]
        widgets = {
            ...
            'status': forms.Select(choices=Subject.STATUS_CHOICES, attrs={
                'class': 'form-control select2',
                'required': True,
            }),
    ...
    def clean(self):
        ...
        if status == 'inactive' and is_active:
            raise forms.ValidationError(_('Môn học không hoạt động không thể ở trạng thái đang hoạt động.'))
        return cleaned_data
