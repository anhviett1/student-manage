...
class TeacherForm(forms.ModelForm):
    class Meta:
        ...
        fields = [
            ...
            'gender', 'address', 'degree', 'specialization', 'years_of_experience',
            'profile_picture', 'bio', 'status', 'is_active', 'department',
        ]
        widgets = {
            ...
            'gender': forms.Select(choices=Teacher.GENDER_CHOICES, attrs={
            ...
            'degree': forms.Select(choices=Teacher.DEGREE_CHOICES, attrs={
                'class': 'form-control select2',
                'required': True,
            }),
    ...
    def clean_phone(self):
        ...
        if Teacher.objects.filter(phone=phone).exclude(pk=self.instance.pk if self.instance else None).exists():
    ...
    def clean(self):
        ...
        if status in ['retired', 'inactive'] and is_active:
            raise forms.ValidationError(_('Giảng viên đã nghỉ hưu hoặc tạm nghỉ không thể ở trạng thái hoạt động.'))
        return cleaned_data
