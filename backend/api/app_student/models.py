...
class Student(models.Model):
    ...
    gpa = models.DecimalField(
        max_digits=3, decimal_places=2, default=0.00, verbose_name="Điểm trung bình"
    )
    ...
    emergency_contact_name = models.CharField(
        max_length=100, verbose_name="Tên người liên hệ khẩn cấp", default="Người thân"
    )
    emergency_contact_phone = models.CharField(
        max_length=15, verbose_name="Số điện thoại khẩn cấp", default="0123456789"
    )
    emergency_contact_relationship = models.CharField(
        max_length=50, verbose_name="Mối quan hệ", default="Gia đình"
    )
    profile_picture = models.ImageField(
        upload_to="student_profiles/", null=True, blank=True, verbose_name="Ảnh đại diện"
    )

    student_id_card = models.CharField(
        max_length=20, unique=True, verbose_name="Mã thẻ sinh viên", default="SV001"
    )
    ...
    class_assigned = models.ManyToManyField(
        "app_class.Class", related_name="student_classes", blank=True, verbose_name="Lớp học"
    )
    subjects = models.ManyToManyField(
        "app_subject.Subject", related_name="student_subjects", blank=True, verbose_name="Môn học"
    )
