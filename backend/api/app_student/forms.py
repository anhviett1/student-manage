...
class StudentForm(forms.ModelForm):
    class Meta:
        ...
        fields = [
            ...
            'gender', 'is_active', 'address', 'city', 'state', 'postal_code', 'country',
            'enrollment_date', 'graduation_date', 'status', 'major', 'minor',
            'emergency_contact_name', 'emergency_contact_phone', 'emergency_contact_relationship',
            'profile_picture', 'student_id_card', 'blood_type', 'medical_conditions',
            'allergies', 'department',
        ]
        widgets = {
            ...
            'gender': forms.Select(choices=Student.GENDER_CHOICES, attrs={
            ...
            'student_id_card': forms.TextInput(attrs={
                'class': 'form-control',
    ...
    def clean_student_id_card(self):
        ...
        if Student.objects.filter(student_id_card=student_id_card).exclude(pk=self.instance.pk if self.instance else None).exists():
    ...
    def clean(self):
        ...
        if status in ['graduated', 'withdrawn'] and is_active:
            raise forms.ValidationError(_('Sinh viên đã tốt nghiệp hoặc rút lui không thể ở trạng thái hoạt động.'))
        return cleaned_data
