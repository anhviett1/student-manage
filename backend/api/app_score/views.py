...
class ScoreViewSet(viewsets.ModelViewSet):
    ...
    def _validate_score(self, instance):
        ...
        if instance.midterm_score is not None and (instance.midterm_score < 0 or instance.midterm_score > 10):
            raise ValueError("Điểm giữa kỳ phải nằm trong khoảng từ 0 đến 10.")
        if instance.final_score is not None and (instance.final_score < 0 or instance.final_score > 10):
            raise ValueError("Điểm cuối kỳ phải nằm trong khoảng từ 0 đến 10.")
        if instance.total_score is not None and (instance.total_score < 0 or instance.total_score > 10):
    ...
    def export(self, request):
        try:
            ...
            headers = [
                "ID", "Sinh viên", "Môn học", "Học kỳ", "Điểm giữa kỳ",
                "Điểm cuối kỳ", "Tổng điểm", "Ghi chú", "Trạng thái",
            ...
            response["Content-Disposition"] = f'attachment; filename="scores_{timezone.now().strftime("%Y%m%d")}.xlsx"'
