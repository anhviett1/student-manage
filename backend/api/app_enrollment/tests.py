...
class EnrollmentModelTest(TestCase):
    def setUp(self):
        ...
        self.student = Student.objects.create(
            student_id="S001", first_name="Test", last_name="Student", email="test@example.com"
        )
        self.class_obj = Class.objects.create(
            class_id="C001", name="Test Class", created_by=self.user
        )
        self.enrollment = Enrollment.objects.create(
            student=self.student, class_obj=self.class_obj, status="active", created_by=self.user
        )
...
class EnrollmentViewTest(TestCase):
    def setUp(self):
        ...
        # Tạo dữ liệu test
        self.student = Student.objects.create(
            student_id="S001", first_name="Test", last_name="Student"
        )
        self.class_obj = Class.objects.create(
            class_id="C001", name="Test Class", department="cntt", created_by=self.user
        )
        self.enrollment = Enrollment.objects.create(
            student=self.student, class_obj=self.class_obj, status="active", created_by=self.user
        )
    ...
    def test_enrollment_create_view(self):
        response = self.client.post(
            ...
            {"student": self.student.id, "class_obj": self.class_obj.id, "status": "active"},
        )
        self.assertEqual(response.status_code, 302)
        self.assertTrue(
            Enrollment.objects.filter(student=self.student, class_obj=self.class_obj).exists()
        )

    def test_enrollment_update_view(self):
        response = self.client.post(
            reverse("enrollment_edit", args=[self.enrollment.pk]),
            {"student": self.student.id, "class_obj": self.class_obj.id, "status": "completed"},
        )
        self.assertEqual(response.status_code, 302)
