<template>
  <div class="enrollment-export-section" aria-label="Phần xuất dữ liệu đăng ký học">
    <Toast aria-live="polite" />
    <ConfirmDialog aria-live="assertive" />
    
    <div class="card">
      <h2>Xuất Dữ Liệu Đăng Ký Học</h2>
      ...
      <div class="export-options">
        ...
        <div class="export-actions">
          <Button
            ...
            @click="exportData"
            class="mr-2"
            aria-label="Xuất dữ liệu đăng ký học"
          />
          <Button
            icon="pi pi-refresh"
            label="Làm mới"
...
const selectedFields = ref(['enrollment_id', 'student_name', 'class_code', 'subject_name', 'semester_name', 'enrollment_date', 'status'])
...
const availableFields = ref([
  { label: 'Mã đăng ký', value: 'enrollment_id' },
  ...
  { label: 'Ngày đăng ký', value: 'enrollment_date' },
  { label: 'Ngày hủy', value: 'cancellation_date' },
  { label: 'Lý do hủy', value: 'cancellation_reason' },
...
const exportData = async () => {
  ...
  try {
    ...
    const response = await api.get(endpoints.enrollmentsExport, { params })
    ...
    link.href = url
    link.download = `enrollments_export_${new Date().toISOString().split('T')[0]}.${getFileExtension(selectedFormat.value)}`
    ...
    toast.add({
      severity: 'success',
      summary: 'Thành công',
      detail: 'Xuất dữ liệu đăng ký học thành công',
      life: 3000
    })
...
const resetFilters = () => {
  ...
  filters.class = null
  filters.status = null
  selectedFields.value = ['enrollment_id', 'student_name', 'class_code', 'subject_name', 'semester_name', 'enrollment_date', 'status']
...
const loadExportHistory = async () => {
  try {
    const response = await api.get(`${endpoints.enrollmentsExport}history/`)
