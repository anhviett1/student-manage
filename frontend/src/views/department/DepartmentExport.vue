<template>
  <div class="department-export-section" aria-label="Phần xuất dữ liệu khoa">
    <Toast aria-live="polite" />
    <ConfirmDialog aria-live="assertive" />
    
    <div class="card">
      <h2>Xuất Dữ Liệu Khoa</h2>
      ...
      <div class="export-filters">
        ...
        <div class="filter-group">
          <label for="type-filter">Loại khoa:</label>
          <Dropdown
            id="type-filter"
...
const selectedFields = ref(['department_code', 'name', 'description', 'dean_name', 'student_count', 'teacher_count', 'status'])
...
const typeOptions = ref([
  { label: 'Tất cả', value: null },
  { label: 'Khoa chính', value: 'main' },
  { label: 'Khoa phụ', value: 'sub' },
  { label: 'Viện nghiên cứu', value: 'institute' }
])
...
const availableFields = ref([
  ...
  { label: 'Tên khoa', value: 'name' },
  { label: 'Mô tả', value: 'description' },
  { label: 'Tên trưởng khoa', value: 'dean_name' },
  { label: 'Mã trưởng khoa', value: 'dean_id' },
  { label: 'Số sinh viên', value: 'student_count' },
  { label: 'Số giảng viên', value: 'teacher_count' },
  { label: 'Số môn học', value: 'subject_count' },
  { label: 'Địa chỉ', value: 'address' },
  { label: 'Số điện thoại', value: 'phone' },
  { label: 'Email', value: 'email' },
  { label: 'Website', value: 'website' },
  { label: 'Loại khoa', value: 'type' },
  { label: 'Trạng thái', value: 'status' },
  { label: 'Ngày thành lập', value: 'established_date' },
...
const exportData = async () => {
  ...
  try {
    ...
    const response = await api.get(endpoints.departmentsExport, { params })
    ...
    link.href = url
    link.download = `departments_export_${new Date().toISOString().split('T')[0]}.${getFileExtension(selectedFormat.value)}`
    ...
    toast.add({
      severity: 'success',
      summary: 'Thành công',
      detail: 'Xuất dữ liệu khoa thành công',
      life: 3000
    })
...
const resetFilters = () => {
  filters.status = null
  filters.type = null
  selectedFields.value = ['department_code', 'name', 'description', 'dean_name', 'student_count', 'teacher_count', 'status']
  selectedFormat.value = 'excel'
}

const loadExportHistory = async () => {
  try {
    const response = await api.get(`${endpoints.departmentsExport}history/`)
