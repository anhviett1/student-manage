<template>
  <div class="card">
    ...
    <TabView>
      ...
      <TabPanel header="Thông Tin Khoa" >
        <div class="profile-section">
          <div class="profile-header">
            <h2>Thông Tin Khoa</h2>
            <Button icon="pi pi-pencil" label="Chỉnh Sửa" severity="primary" @click="openEdit" v-if="!isEditing" v-tooltip="'Chỉnh sửa thông tin khoa'" />
          </div>
      ...
      <TabPanel header="Quản Lý Khoa">
        <div class="header">
          ...
          <div class="action-buttons">
            <Button icon="pi pi-plus" label="Thêm Khoa" severity="primary" class="mr-2" @click="openNew" v-tooltip="'Thêm khoa mới'" />
            <Button icon="pi pi-download" label="Export" severity="success" @click="exportDepartments" v-tooltip="'Xuất danh sách khoa'" />
        ...
        <DataTable v-if="(departments.length > 0 || loading) " :value="departments" :loading="loading" dataKey="department_id" :paginator="true" :rows="10" :rowsPerPageOptions="[5, 10, 20]" responsiveLayout="scroll" class="p-datatable-sm">
          <template #empty>
