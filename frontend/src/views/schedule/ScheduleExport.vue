<template>
  <div class="schedule-export-section" aria-label="Phần xuất dữ liệu lịch học">
    <Toast aria-live="polite" />
    <ConfirmDialog aria-live="assertive" />
    
    <div class="card">
      <h2>Xuất Dữ Liệu Lịch Học</h2>
      ...
      <div class="export-filters">
        ...
        <div class="filter-group">
          <label for="teacher-filter">Giảng viên:</label>
          <Dropdown
            id="teacher-filter"
        ...
        <div class="filter-group">
          <label for="day-filter">Thứ:</label>
          <Dropdown
            id="day-filter"
            v-model="filters.day"
            :options="dayOptions"
            optionLabel="label"
            optionValue="value"
            placeholder="Tất cả thứ"
            class="filter-dropdown"
            aria-label="Lọc theo thứ"
          />
        </div>
      </div>
      ...
      <div class="export-options">
        ...
        <div class="export-actions">
          <Button
            ...
            @click="exportData"
            class="mr-2"
            aria-label="Xuất dữ liệu lịch học"
          />
          <Button
            icon="pi pi-refresh"
            label="Làm mới"
...
const selectedFields = ref(['schedule_id', 'class_code', 'subject_name', 'teacher_name', 'day_of_week', 'start_time', 'end_time', 'room'])
...
const availableFields = ref([
  { label: 'Mã lịch học', value: 'schedule_id' },
  { label: 'Mã lớp', value: 'class_code' },
  { label: 'Tên môn học', value: 'subject_name' },
  { label: 'Mã môn học', value: 'subject_code' },
  { label: 'Tên giảng viên', value: 'teacher_name' },
  { label: 'Mã giảng viên', value: 'teacher_id' },
  { label: 'Thứ trong tuần', value: 'day_of_week' },
  ...
  { label: 'Số tiết học', value: 'periods' },
  { label: 'Loại tiết học', value: 'period_type' },
...
const exportData = async () => {
  ...
  try {
    ...
    const response = await api.get(endpoints.schedulesExport, { params })
    ...
    link.href = url
    link.download = `schedules_export_${new Date().toISOString().split('T')[0]}.${getFileExtension(selectedFormat.value)}`
    ...
    toast.add({
      severity: 'success',
      summary: 'Thành công',
      detail: 'Xuất dữ liệu lịch học thành công',
      life: 3000
    })
...
const resetFilters = () => {
  ...
  filters.day = null
  selectedFields.value = ['schedule_id', 'class_code', 'subject_name', 'teacher_name', 'day_of_week', 'start_time', 'end_time', 'room']
...
const loadClasses = async () => {
  try {
    ...
    classes.value = response.data.results || response.data
  } catch (error) {
...
const loadExportHistory = async () => {
  try {
    const response = await api.get(`${endpoints.schedulesExport}history/`)
