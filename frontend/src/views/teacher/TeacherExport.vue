<template>
  <div class="teacher-export-section" aria-label="Phần xuất dữ liệu giảng viên">
    <Toast aria-live="polite" />
    <ConfirmDialog aria-live="assertive" />
    
    <div class="card">
      <h2>Xuất Dữ Liệu Giảng Viên</h2>
      ...
      <div class="export-options">
        ...
        <div class="export-actions">
          <Button
            ...
            @click="exportData"
            class="mr-2"
            aria-label="Xuất dữ liệu giảng viên"
          />
          <Button
            icon="pi pi-refresh"
            label="Làm mới"
...
const selectedFields = ref(['teacher_id', 'full_name', 'email', 'department', 'position', 'status'])
...
const positionOptions = ref([
  ...
  { label: 'Trưởng khoa', value: 'dean' },
  { label: 'Phó trưởng khoa', value: 'vice_dean' }
])
...
const availableFields = ref([
  ...
  { label: 'Học hàm', value: 'academic_title' },
  { label: 'Học vị', value: 'degree' },
  { label: 'Chuyên ngành', value: 'specialization' },
  { label: 'Ngày vào làm', value: 'hire_date' },
...
const exportData = async () => {
  ...
  try {
    ...
    const response = await api.get(endpoints.teachersExport, { params })
    ...
    link.href = url
    link.download = `teachers_export_${new Date().toISOString().split('T')[0]}.${getFileExtension(selectedFormat.value)}`
    ...
    toast.add({
      severity: 'success',
      summary: 'Thành công',
      detail: 'Xuất dữ liệu giảng viên thành công',
      life: 3000
    })
...
const resetFilters = () => {
  ...
  selectedFields.value = ['teacher_id', 'full_name', 'email', 'department', 'position', 'status']
...
const loadExportHistory = async () => {
  try {
    const response = await api.get(`${endpoints.teachersExport}history/`)
