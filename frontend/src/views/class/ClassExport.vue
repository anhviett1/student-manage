<template>
  <div class="class-export-section" aria-label="Phần xuất dữ liệu lớp học">
    <Toast aria-live="polite" />
    <ConfirmDialog aria-live="assertive" />
    
    <div class="card">
      <h2>Xuất Dữ Liệu Lớp Học</h2>
      ...
      <div class="export-options">
        ...
        <div class="export-actions">
          <Button
            ...
            @click="exportData"
            class="mr-2"
            aria-label="Xuất dữ liệu lớp học"
          />
          <Button
            icon="pi pi-refresh"
            label="Làm mới"
...
const selectedFields = ref(['class_code', 'subject_name', 'teacher_name', 'semester_name', 'capacity', 'enrolled_count', 'status'])
...
const availableFields = ref([
  ...
  { label: 'Số sinh viên đã đăng ký', value: 'enrolled_count' },
  { label: 'Số sinh viên còn lại', value: 'available_slots' },
  { label: 'Trạng thái', value: 'status' },
  { label: 'Thời gian học', value: 'schedule' },
...
const exportData = async () => {
  ...
  try {
    ...
    const response = await api.get(endpoints.classesExport, { params })
    ...
    link.href = url
    link.download = `classes_export_${new Date().toISOString().split('T')[0]}.${getFileExtension(selectedFormat.value)}`
    ...
    toast.add({
      severity: 'success',
      summary: 'Thành công',
      detail: 'Xuất dữ liệu lớp học thành công',
      life: 3000
    })
...
const resetFilters = () => {
  ...
  filters.status = null
  selectedFields.value = ['class_code', 'subject_name', 'teacher_name', 'semester_name', 'capacity', 'enrolled_count', 'status']
...
const loadExportHistory = async () => {
  try {
    const response = await api.get(`${endpoints.classesExport}history/`)
