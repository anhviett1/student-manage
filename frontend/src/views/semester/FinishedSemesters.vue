<template>
  <div class="finished-semesters-section" aria-label="Phần học kỳ đã kết thúc">
    <Toolbar class="toolbar">
      <template #start>
        <h2>Học Kỳ Đã Kết Thúc</h2>
      </template>
      <template #end>
        <Button
          icon="pi pi-filter"
          label="Học Kỳ Không Active"
          severity="info"
          class="mr-2"
          @click="emit('loadSemesters', 1, paginatorInfo.rows, false)"
          v-tooltip="'Xem các học kỳ không hoạt động'"
          aria-label="Xem các học kỳ không hoạt động"
        />
      </template>
    </Toolbar>
    <div class="filter-bar">
      <Dropdown
        ...
        @change="emit('loadSemesters', 1, paginatorInfo.rows, false)"
    ...
    <DataTable
      ...
      aria-label="Bảng học kỳ đã kết thúc"
    >
      <template #empty>
...
const debouncedLoadSemesters = debounce(() => emit('loadSemesters', 1, props.paginatorInfo.rows, false), 500);

onMounted(() => {
  gsap.from('.finished-semesters-section', { opacity: 0, y: 20, duration: 0.5 });
});

const onPage = (event) => {
  emit('loadSemesters', event.page + 1, event.rows, false);
};
